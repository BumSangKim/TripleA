...
def test_decision_logging_and_score_store_round_trip():
    ...
    writer = SqliteStrategyDecisionLogRepository(conn)
    assert log_strategy_decision(writer, enabled=False, decision_id="d0", as_of_date=date(2026, 5, 27), decision_type="x", payload={}) is False
    assert log_strategy_decision(writer, enabled=True, decision_id="d1", as_of_date=date(2026, 5, 27), decision_type="x", payload={"a": 1}, reason_codes=["ok"]) is True
