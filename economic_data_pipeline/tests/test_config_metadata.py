...
def test_indicators_have_required_fields():
    data = yaml.safe_load((BASE_DIR / "config" / "indicators.yaml").read_text(encoding="utf-8"))
...
def test_economic_events_yaml_contains_required_events():
    data = yaml.safe_load((BASE_DIR / "config" / "economic_events.yaml").read_text(encoding="utf-8"))
