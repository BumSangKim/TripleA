...
class SQLiteScoreStore:
    ...
    def create_run(self, run_id: str, feature_snapshot_id: str, as_of_date: date, event_profile: str, parameter_version: str, model_version: str, status: str, warnings: list[str]) -> None:
        ...
        self.conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (run_id, feature_snapshot_id, as_of_date.isoformat(), event_profile, parameter_version, model_version, status, json.dumps(warnings), now, now),
        )
        self.conn.commit()

    def insert_value(self, run_id: str, output: ScoreOutput) -> None:
        data = asdict(output)
        self.conn.execute(
            """
    ...
    def lookup_previous_score(self, score_key: str, subject_type: str, subject_id: str, before_date: date) -> float | None:
...
class ScoreRunner:
    def __init__(
        ...
        definitions: dict[str, ScoreDefinition],
        event_profiles: dict[str, dict[str, Any]],
        store: SQLiteScoreStore | None = None,
    ):
