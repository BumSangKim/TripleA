...
class CommonSectorScoringEngine:
    ...
    def score_sector(self, sector_code: str, benchmark_asset_code: str | None, market_asset_code: str | None, as_of_date: date) -> CommonSectorScore:
        ...
        prices = _prices(self.conn, benchmark_asset_code, as_of_date)
        market_prices = _prices(self.conn, market_asset_code, as_of_date) if market_asset_code else []
        if len(prices) < 2:
...
def _prices(conn: sqlite3.Connection, asset_code: str | None, as_of_date: date) -> list[float]:
    if not asset_code:
        return []
    rows = conn.execute(
        """
        ...
        """,
        (asset_code, as_of_date.isoformat()),
    ).fetchall()
